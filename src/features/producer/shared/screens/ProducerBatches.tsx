    ...
    <ProducerScreenShell product={product} eyebrow={product.name} title={`Lô sản xuất ${product.name}`}>
      <div className="grid gap-3">
        {product.data.batches.map((batch) => (
          <Link key={batch.id} to={`/producer/${product.key}/batch/${batch.id}`} className="rounded-lg border border-[#E0C69B] bg-white p-4 shadow-sm transition hover:border-gold">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold text-[#8A6238]">{batch.id}</p>
                <h2 className="mt-1 text-lg font-bold">{batch.name}</h2>
                <p className="mt-1 text-sm text-[#6F4B35]">{batch.notes}</p>
              </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
              <div><p className="text-[#8B7460]">Số lượng</p><strong>{batch.quantity}</strong></div>
              <div><p className="text-[#8B7460]">Hoàn tất</p><strong>{batch.expectedAt}</strong></div>
              <div><p className="text-[#8B7460]">AI score</p><strong>{batch.qualityScore}/100</strong></div>
            </div>
          </Link>
        ))}
      </div>
