...
export function ProducerBatchDetail({ product }: ProducerBatchDetailProps) {
  ...
  return (
    <ProducerScreenShell product={product} eyebrow={product.name} title={batch.name}>
      <Link to={product.routes.batches} className="mb-4 inline-flex text-sm font-medium text-[#8a6417]">
      ...
      <section className="rounded-lg border border-[#ead7a4] bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-[#7a5a3a]">{batch.notes}</p>
          </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-4">
          <div><p className="text-sm text-[#8a7a65]">Bắt đầu</p><strong>{batch.startedAt}</strong></div>
          <div><p className="text-sm text-[#8a7a65]">Dự kiến</p><strong>{batch.expectedAt}</strong></div>
          <div><p className="text-sm text-[#8a7a65]">Số lượng</p><strong>{batch.quantity}</strong></div>
          <div><p className="text-sm text-[#8a7a65]">AI score</p><strong>{batch.qualityScore}/100</strong></div>
        </div>
      </section>
