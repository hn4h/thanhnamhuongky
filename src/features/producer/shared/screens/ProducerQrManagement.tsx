...
export function ProducerQrManagement({ product }: ProducerQrManagementProps) {
  return (
    <ProducerScreenShell product={product} eyebrow={product.name} title={`QR / serial ${product.name}`}>
      <div className="overflow-hidden rounded-lg border border-[#ead7a4] bg-white shadow-sm">
        {product.data.qrCodes.map((qr) => (
          <div key={qr.id} className="grid gap-2 border-b border-[#f0dfad] p-4 last:border-b-0 md:grid-cols-4">
            <div>
              <p className="text-xs text-[#8a7a65]">QR ID</p>
              <strong>{qr.id}</strong>
            </div>
            <div>
              <p className="text-xs text-[#8a7a65]">Batch</p>
              <strong>{qr.batchId}</strong>
            </div>
            <div>
              <p className="text-xs text-[#8a7a65]">Serial</p>
              <strong>{qr.serial}</strong>
            </div>
            <div>
              <p className="mb-1 text-xs text-[#8a7a65]">Trạng thái</p>
