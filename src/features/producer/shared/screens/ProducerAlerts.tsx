    ...
    <ProducerScreenShell product={product} eyebrow={product.name} title={`Cảnh báo ${product.name}`}>
      <div className="grid gap-3">
        {product.data.alerts.map((alert) => (
          <article key={alert.id} className="rounded-lg border border-[#E0C69B] bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold text-[#8A6238]">{alert.createdAt}</p>
            ...
            <p className="mt-3 text-sm leading-6 text-[#6F4B35]">{alert.message}</p>
          </article>
        ))}
      </div>
    </ProducerScreenShell>
