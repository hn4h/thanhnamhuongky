    ...
    <ProducerScreenShell product={product} eyebrow={product.name} title={`Quy trình ${product.name}`}>
      <div className="grid gap-3 md:grid-cols-3">
        {steps.map((step, index) => (
          <div key={step} className="rounded-lg border border-[#E0C69B] bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-[#6F4B35]">Theo dõi trạng thái {step.toLowerCase()} riêng cho {product.name}, dùng mock data trong frontend.</p>
          </div>
        ))}
      </div>
    </ProducerScreenShell>
