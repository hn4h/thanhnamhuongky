  ...
  return (
    <ConsumerShell title="Hồ sơ hộp bánh gai" subtitle="Thông tin sản phẩm, lô sản xuất và hạn dùng.">
      <section className="rounded-lg border border-[#E0C69B] bg-white p-5 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#8A6238]">{product.grade}</p>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          {Object.entries({
            ...
            <div key={label} className="rounded-md bg-[#FBF3E7] p-3">
              <p className="text-xs text-[#8B7460]">{label}</p>
              <strong>{value}</strong>
            </div>
          ))}
        </div>
